...
var uaList = []string{
	...
	"Mozilla/5.0 (Windows NT 10.0; Win64; rv:46.0) Gecko/20100101 Firefox/46.0",
	"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36 OPR/37.0.2178.32",
}
...
func ruleResource() []resource {
	...
	r2 := resource{ // 列表页
		url:    "http://localhost:8888/list/1.html",
		target: "{$id}",
		start:  1,
		end:    21,
	}
...
func makeLog(current, refer, ua string) string {
	...
	logTemplate := "127.0.0.1 - - [08/Mar/2018:00:48:34 +0888] \"OPTIONS /dig?{$paramsStr} HTTP/1.1\" 200 43 \"-\" \"{$ua}\" \"-\""
...
func main() {
	...
	filePath := flag.String("filePath", "D://MyApplication//goland//goproject//logAnalysis//logs//dig.log", "log file path")
	flag.Parse()
